...
class Login(UserPageBase):
	...
	def renderPage(self):
		#logging.info("userData:"+self.userData.data+" passData:"+self.passData.data+" emailData:"+self.emailData.data)
		self.render("login.html", userData = self.userData, passData = self.passData)
...
class Welcome(UserPageBase):
	def get(self):
		logging.info(str(self.current_user))
		if not self.current_user:
			self.redirect("/signup");
		else:
			self.render("welcome.html", username = self.current_user.name)

class Logout(UserPageBase):
	def get(self):
		self.logout()
		self.redirect('/signup')